...
import { Prediction, PredictionOverUnder, MarketPick } from '../predictions/extractor';
 ...
 */
export function transformToSafePick(pred: Prediction): SafePick {
  ...
  let safePick: string;
  let safeMarket: string;
  let safeOdds: number | null;
  let reason: string;
 ...
 */
export function transformPredictions(predictions: Prediction[]): SafePick[] {
  return predictions
 ...
 */
export function calculateBetPlan(
  picks: SafePick[],
  totalBalance: number,
  minimumBet: number = 50, // Msport Nigeria minimum is ₦50
): BetPlan {
 ...
 */
function estimateSafeOdds(confidence: number, market: string, line: number): number {
  // Very safe predictions have very low odds
  if (confidence >= 0.99) return 1.02;
  if (confidence >= 0.97) return 1.04;
  if (confidence >= 0.95) return 1.07;
  if (confidence >= 0.90) return 1.10;
...
function findLowestSafeOver(ouPreds: PredictionOverUnder[]): PredictionOverUnder | null {
  // Sort ascending by line, find first with over ≥ 95%
  const sorted = [...ouPreds].sort((a, b) => a.line - b.line);
  for (const ou of sorted) {
    if (ou.over >= MIN_CONFIDENCE_DISPLAY) return ou;
  }
  return null;
}

/** Find the highest Under line with ≥95% confidence */
function findHighestSafeUnder(ouPreds: PredictionOverUnder[]): PredictionOverUnder | null {
  // Sort descending by line, find first with under ≥ 95%
  const sorted = [...ouPreds].sort((a, b) => b.line - a.line);
  for (const ou of sorted) {
    if (ou.under >= MIN_CONFIDENCE_DISPLAY) return ou;
  }
  return null;
}
