...
import { MsportAutomation, MsportConfig } from './msport/automation';
import { createBot, registerSkipHandler } from './bot/index';
import { startDailyPush } from './bot/daily-push';
...
async function main(): Promise<void> {
  ...
  const cache = new SqliteCache(CACHE_PATH);
  // Purge expired entries on startup
  cache.purge();

  // Create subscriber store using same DB
  const rawDb = new BetterSqlite3(CACHE_PATH);
  rawDb.pragma('journal_mode = WAL');
  const subscribers = new SubscriberStore(rawDb);

  // 2. Initialize Score180 client
  console.log('[main] Initializing Score180 client...');
  const client = new Score180Client({
  ...
  const service = new PredictionsService(client, cache);
  ...
  const msportConfig: MsportConfig = {
    phone: MSPORT_PHONE,
    password: MSPORT_PASSWORD,
    headless: true, // Always headless in production
  };
  const msport = new MsportAutomation(msportConfig);
